...
// Processa a adição de um administrador
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['adicionar_admin'])) {
    ...
    if (empty($nome) || empty($email) || empty($senha) || empty($telefone)) {
        ...
        // Normaliza o email: remove espaços extras e verifica o domínio
        $email = strtolower(trim($email));
        $domain = '@mercadobompreco.com';

        // Verifica se o email contém mais de um '@'
...
// Processa a exclusão de um administrador
if (isset($_GET['delete_admin'])) {
    $admin_id = $_GET['delete_admin'];

    // Evita que o admin logado se exclua
...
$sql_admins = "SELECT id, nome, email FROM usuarios WHERE tipo = 'admin' ORDER BY nome";
        ...
        <div class="add-admin-content">
            ...
            <div class="add-admin-section">
                ...
                <form class="add-admin-form" method="POST" action="add_admin.php">
                    ...
                    <input type="email" name="email" id="email" placeholder="exemplo@mercadobompreco.com" required>
